...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggleMenu = (show) => {
    if (navMenu) navMenu.classList.toggle('show-menu', show);
  ...
  // ===================================================================
  // --- 6. MOBILE CATEGORY SLIDESHOW (SWIPER.JS) ---
  // ===================================================================
  function initializeCategorySlider() {
    // Check if the slider element exists and we are on a mobile-sized screen
    if (window.innerWidth <= 767 && document.querySelector('#category-slider-mobile')) {
      const swiper = new Swiper('#category-slider-mobile', {
        // ✅ Makes the slider loop infinitely
        loop: true,
        
        // ✅ Makes the slideshow automatic
        autoplay: {
          delay: 3000, // Time in milliseconds between slides
          disableOnInteraction: false, // Autoplay will resume after user interaction
        },
        
        // Shows 1 full slide and a little bit of the next one to encourage swiping
        slidesPerView: 1.3, 
        spaceBetween: 15, // Gap between slides
        centeredSlides: true, // The active slide will be centered

        // Enables the pagination dots at the bottom
        pagination: {
  ...
  // ===================================================================
  // --- 7. DYNAMIC PRODUCT LOADER WITH PAGINATION ---
  // ===================================================================
  ...
  // --- FUNCTION TO CREATE AND DISPLAY PAGINATION LINKS ---
